...
- (id) initWithFrame:(CGRect)theFrame cancelEnabled:(BOOL)isCancelEnabled oneDMode:(BOOL)isOneDModeEnabled {
  ...
  if( self ) {
    ...
    UIImageView *view1 = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"scan1.png"]];
      view1.center = CGPointMake(theFrame.origin.x + theFrame.size.width/2, theFrame.origin.y + theFrame.size.height/2 - view1.frame.size.height*2 );
      [self addSubview:view1];
      [view1 release];
    UIImageView *view2 = [[UIImageView alloc]initWithImage:[UIImage imageNamed:@"scan2.png"]];
    view2.center = CGPointMake(theFrame.origin.x + theFrame.size.width/2, theFrame.origin.y +theFrame.size.height - view2.frame.size.height/2);
...
- (void)drawRect:(CGRect)rect {
  ...
  if (displayedMessage == nil) {
    self.displayedMessage = @"将镜头对准条码/二维码\n并使他们置于下方的方框内.";
  }
  ...
  //	CGContextSetStrokeColor(c, white);
    ...
    CGSize displaySize = [self.displayedMessage sizeWithFont:font constrainedToSize:constraint];
        CGRect displayRect = CGRectMake((rect.size.width - displaySize.width) / 2 , 10, displaySize.width, displaySize.height);
